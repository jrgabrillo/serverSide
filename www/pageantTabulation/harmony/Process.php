...
if(isset($_GET['getCandidate'])){
	$data = $_POST['data'];
	$list = [];
	$query = $function->PDO(true,"SELECT DISTINCT(category) FROM tbl_contestant WHERE status='1' ORDER BY category ASC");
...
if(isset($_GET['getCandidateWithScores'])){
	$data = $_POST['data'];
	$list = [];
	$query = $function->PDO(true,"SELECT DISTINCT(category) FROM tbl_contestant WHERE status='1'");
	foreach ($query as $key => $value) {
		$query2 = $function->PDO(true,"SELECT * FROM tbl_contestant WHERE category='{$value[0]}' ORDER BY name ASC");
		$subList = [];
